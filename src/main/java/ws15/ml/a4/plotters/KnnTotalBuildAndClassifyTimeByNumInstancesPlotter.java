...
public class KnnTotalBuildAndClassifyTimeByNumInstancesPlotter implements Consumer<List<KnnEvaluation>> {

    private final Configuration configuration;

    public KnnTotalBuildAndClassifyTimeByNumInstancesPlotter(Configuration configuration) {
    ...
    public void accept(List<KnnEvaluation> evaluations) {
        Chart chart = new ChartBuilder()
                ...
                .width(CHART_WIDTH)
                .height(CHART_HEIGHT)
                .title("Total build + classify time by number of instances per strategy")
                .xAxisTitle("# instances")
    ...
    private List<Integer> getXAxisLabels(List<KnnEvaluation> evaluations) {
        return evaluations.stream()
                .map(it -> it.getInstances().numInstances())
                .sorted()
    ...
    private List<Long> getYAxisData(List<KnnEvaluation> evaluations) {
        return evaluations.stream()
                .sorted((o1, o2) -> o1.getInstances().numInstances() - o2.getInstances().numInstances())
                .map(it -> it.getBuildTimeMs() + it.getClassificationTimeMs())
                .collect(Collectors.toList());
    }

    private String getOutputFileName() {
        return new File(configuration.getOutputDir(), "total-build-and-classify-time-by-num-instances.png").getAbsolutePath();
    }
}
